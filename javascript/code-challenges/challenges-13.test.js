...
const longestString = (arr) => {
  let newArr = arr.reduce((acc, cur) => acc.length < cur.length ? acc = cur : acc, '');
...
const firstLetters = (arr) => {
 let newArr2 = arr.map(str => str.charAt(0));
  return newArr2;
};
...
const findHappiness = (arr) => {
  return arr.filter(string3 => string3.includes(':)'));
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(item => item.split('').filter(num => /[0-9]/.test(num)).join(''));
};
...
const allHappy = (arr) => {
  let newArr6 = [];
  newArr6 = arr.filter(val =>
    val.includes(":)")
  )
  return newArr6.length === arr.length ? true : false
};
