...
const toLastNames = people => {
  ...
  newArray1.push([num.firstName , num.lastName].join(' '))
...
let newArray2=arr.reduce((acc,value)=>{
return acc +value;
},0);
return newArray2;
};
...
let newArray3 = arr.reduce((acc,value)=>{
...
let numOfElem = arr.reduce((acc,v)=>{
return acc +1 ;
},0);
...
let newArray5= arr.reduce((acc,val,i)=>{
  acc[i]=val.name ;
  return acc;
},[])
return newArray5;
...
const reversedString = (str) => {
 let newArray6=str.split("").reduce((acc,val,index)=>{
  acc= val+acc ,"";
   return acc;
 })
 return newArray6
