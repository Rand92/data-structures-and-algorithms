...
const count = (target, input) => {
  let countArr = input.map(item => item.filter(x => x == target).length).reduce((a, b) => a + b, 0);
  return countArr;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let newArr = input.map(item => item.filter(x =>typeof(x) === "number"&& x % 5 == 0  ));
  let output = newArr.map(ele=>ele.map(element=>Math.pow(2,element)))
  return output;
};
...
let findMaleAndFemale = (data) => {
  let genderOutput = data.map(item => item.gender === 'female' || item.gender === 'male'? item.name : '')
  return genderOutput.filter(String).join(' and ')
};
